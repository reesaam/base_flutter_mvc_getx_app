...
import '../model/repositories/update_repositories.dart';
import '../model/repositories/update_repositories_impl.dart';

class UpdateController extends CoreController {
  final UpdateRepository _updateRepository = const UpdateRepositoryImpl();
  ...
  _downloadAction() async {
    ...
    if (dlDir != null && dlFile != null) {
      ...
      final resultAddress = await _updateRepository.getDownloadAddress();
      resultAddress.fold((l) => showErrorDialog(message: l.toString()), (r) => downloadAddress = r);

      if (downloadAddress.isNotEmpty) {
        final result = await _updateRepository.updateDownload(savePath: dlFile!.path);
